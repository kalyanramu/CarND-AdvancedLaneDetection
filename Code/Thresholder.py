...
class Thresholder:
    ...
    def dir_thresh(self, sobelx, sobely):
        ...
        sxbinary[(scaled_sobel >= self.thresh_dir_min) &
                 (scaled_sobel <= self.thresh_dir_max)] = 1
    ...
    def mag_thresh(self, sobelx, sobely):
        ...
        binary_output[(gradmag >= self.thresh_mag_min) &
                      (gradmag <= self.thresh_mag_max)] = 1
    ...
    def threshold(self, img):
        sobelx = cv2.Sobel(img[:, :, 2], cv2.CV_64F, 1,
                           0, ksize=self.sobel_kernel)
        sobely = cv2.Sobel(img[:, :, 2], cv2.CV_64F, 0,
                           1, ksize=self.sobel_kernel)
